...
export default function Unwrap(props:any) {
  const { unwrap, fetchTokenBalance, calculateGasFee } = useWeb3Functions();
  ...
  const submit = async () => {
    ...
    setLoading(true);
    const recepit = await unwrap(amount, token, toChain);
...
<h2 className="text-xl font-extralight text-gray-400 lg:text-3xl cursor-pointer" onClick={() =>{
  props.setIsUnwrap(false);
  props.setIsWrap(false);
}}>Bridge</h2>
  <h2 className="text-xl font-bold lg:text-4xl px-4  cursor-pointer" onClick={()=> props.setIsUnwrap(true)}>Unwrap</h2>

  <h2 className="text-xl text-gray-400 lg:text-3xl px-4 font-extralight cursor-pointer" onClick={()=> props.setIsWrap(true)}>Wrap</h2>
</div>
