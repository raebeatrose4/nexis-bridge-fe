...
export default function Wrap(props:any) {
  const { wrap, fetchTokenBalance, calculateGasFee } = useWeb3Functions();
  ...
  const [token, setToken] = useState<Token | null>(tokens[1]);
  console.log(token)
  ...
  const submit = async () => {
    ...
    setLoading(true);
    const recepit = await wrap(amount, token, toChain);
...
<h2 className="text-xl font-extralight text-gray-400 lg:text-3xl cursor-pointer" onClick={() =>{
  ...
  <h2 className="text-xl font-extralight text-gray-400 lg:text-3xl cursor-pointer" onClick={()=> {
    props.setIsUnwrap(true)
    props.setIsWrap(false)
    }}>Unwrap</h2>

  <h2 className="text-xl font-bold lg:text-4xl px-4  cursor-pointer" onClick={()=> props.setIsWrap(true)}>Wrap</h2>
</div>
